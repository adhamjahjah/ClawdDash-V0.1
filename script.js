...
function renderHome() {
  ...
  const upcoming   = [...SCHEDULED].sort((a, b) => a.next_run - b.next_run).slice(0, 3);
  ...
  upcomingSec.innerHTML = `
    <div class="widget-header">
      ...
      <div class="widget-header-right">
        <span class="widget-meta">${SCHEDULED.length} tasks</span>
...
function renderScheduled() {
  ...
  const keys = Object.keys(grouped).sort((a, b) => {
    const da = SCHEDULED.find(s => fmtDate(s.next_run) === a).next_run;
    const db = SCHEDULED.find(s => fmtDate(s.next_run) === b).next_run;
    return da - db;
  });
  ...
  keys.forEach(dk => {
    ...
    grouped[dk].forEach(sch => {
      ...
      item.querySelector('input[type=checkbox]').addEventListener('change', e => {
        const s = SCHEDULED.find(x => x.id === e.target.dataset.id);
        if (s) { s.enabled = e.target.checked; if (tg) tg.HapticFeedback?.impactOccurred('light'); }
      });
...
function init() {
  ...
  setInterval(simulateProgress, 1700);  // progress ticks
